...
function Installer([string]$mode){
    $arguments=@('run','--rm','--network','none','--entrypoint','python','-e','PYTHONPATH=/app/web/ops/xhs_hermes',
        ...
        $tag,'/release/install_hermes_policy.py')
...
if(([regex]::Matches($raw,'ztqc/hermes-worker:0\.3\.22')).Count -ne 1){throw 'Unexpected existing Worker image'}
[IO.File]::WriteAllText($candidate,$raw.Replace('ztqc/hermes-worker:0.3.22',$tag),$utf8)
$before=Config $live
...
if($LASTEXITCODE -ne 0 -or $image.Config.Labels.'org.opencontainers.image.revision' -ne 'policy-20260909-b9b93a75'){throw 'Wrong candidate build'}
Installer ''
...
$record=@{version='0.3.23';scope='policy-and-policy-constraints-only';candidateHash=(Get-FileHash $candidate).Hash;
