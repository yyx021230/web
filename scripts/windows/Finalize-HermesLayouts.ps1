...
$record = Get-Content "$release\release-receipt.json" -Raw | ConvertFrom-Json
...
$record.status = if ($complete) {'live-acceptance-passed'} else {'live-content-acceptance-pending'}
foreach ($item in @{
    ...
    acceptance=$accept;contentAcceptancePassed=$complete;backupsCopiedToMac=$false;
...
$core = Get-Content "$project\.release\current.json" -Raw | ConvertFrom-Json
...
$core.localFrontend.version = '0.3.21'
$core.localFrontend | Add-Member -NotePropertyName image -NotePropertyValue $record.images.frontend -Force
[IO.File]::WriteAllText("$project\.release\current.json", ($core | ConvertTo-Json -Depth 20), $utf8)
@{moduleVersion=$record.version;status=$record.status;contentAcceptancePassed=$complete;coreVersion=$core.version;receipt="$project\.release\hermes\current.json"} | ConvertTo-Json
