...
export default function HermesTypePicker({ kind, catalog, value, quoteAllowed, onSelect, onClose }: {
  ...
  const pageSize = kind === 'image' ? 6 : 3;
  const pageCount = Math.max(1, Math.ceil((selected?.examples.length || 0) / pageSize));
  const visibleExamples = selected?.examples.slice(page * pageSize, (page + 1) * pageSize) || [];
  const blocked = kind === 'image' && (selected?.requires_quote_data || ['quote_table', 'quote_cards'].includes(active)) && !quoteAllowed;
  const eligible = kind === 'image' ? (quoteAllowed ? selected?.production_reference_count : selected?.production_without_quote_count) : undefined;
  const noReferences = !selected?.reference_count || eligible === 0;
  return <HermesModal title={kind === 'copy' ? '选择文案方向' : '选择图片版式'} description="文案与图片独立选择，自由组合。" onClose={onClose} bodyClassName={s.typeDialogBody}
    footer={<><span className={s.muted}>{blocked ? '该类型的母版含多配置报价，需完整配置价格。' : noReferences ? '当前资料条件下暂无可生产母版，可继续浏览原图。' : eligible != null ? `${eligible} 套候选母版 · 生成时继续校验内容` : '只从所选类型匹配；实际可用数量还需通过生产校验。'}</span><button className={s.primary} disabled={!selected || blocked || noReferences} onClick={() => selected && onSelect(selected)}><Check size={15} />使用{selected?.name || '此类型'}</button></>}>
    <div className={s.typeBrowser}>
      ...
      <section className={s.typeResults}>{selected && <>
        ...
        <div className={s.typeExamplesHeading}><strong>{catalog.source === 'synced_online_library' ? '线上库真实案例' : '库内真实案例'}</strong><span>{selected.reference_count} 套版式参考 · {selected.examples.length} 条可浏览</span></div>
        {!selected.examples.length && <div className={s.typeEmpty}><ImageIcon size={22} strokeWidth={1.4} /><strong>{selected.reference_count ? '原图暂不可用' : '当前库内暂无此类型母版'}</strong><p>不借用其他类型的图片补位。上方仅为结构示意。</p></div>}
        <div className={kind === 'image' ? s.imageExamples : s.copyExamples}>{visibleExamples.map(example => <ExampleCard key={`${selected.id}-${example.id}`} example={example} expanded={expanded === example.id} onExpand={() => setExpanded(expanded === example.id ? null : example.id)} />)}</div>
        {pageCount > 1 && <nav className={s.typePagination} aria-label="案例分页"><button className={s.button} disabled={page === 0} onClick={() => { setPage(p => p - 1); setExpanded(null); }}>上一页</button><span>{page + 1} / {pageCount}</span><button className={s.button} disabled={page + 1 >= pageCount} onClick={() => { setPage(p => p + 1); setExpanded(null); }}>下一页</button></nav>}
...
function ExampleCard({ example, expanded, onExpand }: { example: HermesReferenceExample; expanded: boolean; onExpand: () => void }) {
  ...
  return <article className={s.exampleCard}>
    ...
    <div className={s.exampleBody}><div className={s.between}><span className={s.eyebrow}>{example.source_task_id ? `历史任务 #${example.source_task_id}` : `${example.kind === 'copy' ? '文案库' : '提示词库'} #${example.id}`}</span><span className={s.small}>{example.visual_reviewed ? '版式已核对' : '原始参考'}</span></div><h4>{example.title}</h4>{example.requires_quote_data && <p className={s.small}>此母版含多配置报价 · 需完整价格资料</p>}{example.classification_reason && <details className={s.sourceNote}><summary>归类依据{example.style_tags?.length ? ` · ${example.style_tags.join(' / ')}` : ''}</summary><p>{example.classification_reason}</p></details>}<div className={s.exampleCopy} data-expanded={expanded}>{example.content}</div><button className={s.textButton} onClick={onExpand}>{expanded ? '收起' : example.kind === 'copy' ? '读完整原文与话题' : '查看完整原始提示词'}<ArrowRight size={12} /></button></div>
  </article>;
}
