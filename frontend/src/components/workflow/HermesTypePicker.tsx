...
export default function HermesTypePicker({ kind, catalog, value, quoteAllowed, onSelect, onClose }: {
  ...
  const noReferences = !selected?.reference_count;
  return <HermesModal title={kind === 'copy' ? '选择文案方向' : '选择图片版式'} description="文案与图片独立选择，自由组合。" onClose={onClose} bodyClassName={s.typeDialogBody}
    footer={<><span className={s.muted}>{blocked ? '该类型的母版含多配置报价，需完整配置价格。' : noReferences ? '当前库内暂无这一类型的母版。' : '只从所选类型匹配；实际可用数量还需通过生产校验。'}</span><button className={s.primary} disabled={!selected || blocked || noReferences} onClick={() => selected && onSelect(selected)}><Check size={15} />使用{selected?.name || '此类型'}</button></>}>
    <div className={s.typeBrowser}>
      ...
      <section className={s.typeResults}>{selected && <>
        ...
        <div className={s.typeExamplesHeading}><strong>{catalog.source === 'synced_online_library' ? '线上库真实案例' : '库内真实案例'}</strong><span>{selected.reference_count} 条归类参考 · 展示 {selected.examples.length} 条</span></div>
...
function ExampleCard({ example, expanded, onExpand }: { example: HermesReferenceExample; expanded: boolean; onExpand: () => void }) {
  ...
  return <article className={s.exampleCard}>
    ...
    <div className={s.exampleBody}><div className={s.between}><span className={s.eyebrow}>{example.kind === 'copy' ? '文案库' : '提示词库'} #{example.id}</span><span className={s.small}>原始参考</span></div><h4>{example.title}</h4>{example.classification_reason && <details className={s.sourceNote}><summary>归类依据{example.style_tags?.length ? ` · ${example.style_tags.join(' / ')}` : ''}</summary><p>{example.classification_reason}</p></details>}<div className={s.exampleCopy} data-expanded={expanded}>{example.content}</div><button className={s.textButton} onClick={onExpand}>{expanded ? '收起' : example.kind === 'copy' ? '读完整原文与话题' : '查看完整原始提示词'}<ArrowRight size={12} /></button></div>
  </article>;
}
