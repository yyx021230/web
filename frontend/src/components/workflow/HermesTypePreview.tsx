...
export const PREVIEW_TYPES = [
  'hero', 'headline', 'collage', 'table', 'cards', 'list', 'dialog', 'diagram',
...
export default function HermesTypePreview({ id, small = false }: { id: string; small?: boolean }) {
  const layouts: Record<string, ReactNode> = {
    list: sheet(<>{lines(22, 20, 74, 1)}{[40, 66, 92].map((y, i) => <g key={y}><text x="21" y={y + 9} fontSize="11" fill="currentColor">{i + 1}.</text>{lines(36, y + 2, 59, 3)}</g>)}{lines(22, 124, 65, 1)}</>),
    dialog: sheet(<>{[24, 57, 90].map((y, i) => <g key={y}><circle cx={i % 2 ? 96 : 23} cy={y + 5} r="5" fill="currentColor" opacity=".4" /><rect x={i % 2 ? 22 : 35} y={y} width="52" height="26" rx="5" fill="currentColor" opacity={i % 2 ? '.25' : '.1'} />{lines(i % 2 ? 28 : 41, y + 7, 39, 2)}</g>)}</>),
  ...
  const primary: Record<string, string> = { hero: 'scene_poster', headline: 'headline_poster', collage: 'photo_collage', table: 'quote_table', cards: 'quote_cards', diagram: 'feature_infographic' };
  return <span className={s.typePreview} data-small={small} data-type-preview={id} aria-hidden="true"><svg viewBox="0 0 120 144" focusable="false">{layouts[primary[id] || id] || sheet(lines(22, 25, 72, 10))}</svg></span>;
}
