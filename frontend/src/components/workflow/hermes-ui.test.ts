...
describe('Hermes creation frontend interactions', () => {
  ...
  it('provides distinct schematic previews for all 7 copy and 8 primary image directions', async () => {
  ...
  it('pages all real examples and resets the page when switching type', async () => {
    const examples = Array.from({ length: 9 }, (_, i) => ({ id: i + 1, kind: 'image' as const, title: `真实样本${i + 1}`, content: `原文${i + 1}`, source_task_id: 8000 + i, visual_reviewed: true }));
    const cards = { ...reference('cards', '分块卡片'), reference_count: 9, examples, production_reference_count: 4, production_without_quote_count: 2 };
    const dialog = { ...reference('dialog', '对话问答'), examples: [examples[0]] };
    const catalog = { version: 'v4', source: 'test', library_counts: { copy: 0, image: 10 }, copy_types: [], image_types: [cards, dialog], source_note: '' };
    await render(React.createElement(HermesTypePicker, { kind: 'image', catalog, value: 'cards', quoteAllowed: false, onSelect: vi.fn(), onClose: vi.fn() }));
    expect(host.querySelectorAll('article')).toHaveLength(6);
    expect(host.textContent).toContain('2 套候选母版');
    expect(host.textContent).toContain('历史任务 #8000');
    await click(button('下一页'));
    expect(host.querySelectorAll('article')).toHaveLength(3);
    expect(host.textContent).toContain('真实样本9');
    await click(host.querySelector('[aria-label="对话问答"]'));
    expect(host.querySelectorAll('article')).toHaveLength(1);
    expect(host.textContent).toContain('真实样本1');
  });

  it('keeps samples visible but disables a verified empty production pool', async () => {
    const type = { ...reference('table', '表格对照'), production_reference_count: 6, production_without_quote_count: 0 };
    const catalog = { version: 'v4', source: 'test', library_counts: { copy: 0, image: 1 }, copy_types: [], image_types: [type], source_note: '' };
    await render(React.createElement(HermesTypePicker, { kind: 'image', catalog, value: 'table', quoteAllowed: false, onSelect: vi.fn(), onClose: vi.fn() }));
    expect(host.querySelectorAll('article')).toHaveLength(1);
    expect(button('使用表格对照')).toHaveProperty('disabled', true);
    expect(host.textContent).toContain('当前资料条件下暂无可生产母版');
  });
  ...
  it('keeps card quote prices gated and shows actual classification evidence', async () => {
    ...
    expect(host.textContent).toContain('3 套版式参考');
    expect(onSelect).not.toHaveBeenCalled();
  });
