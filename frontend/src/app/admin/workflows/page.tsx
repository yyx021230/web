...
export default function AdminWorkflowsPage() {
  ...
  const fetchWorkflows = useCallback(async () => {
    try {
      const res = await adminApi.getAdminWorkflows(1, 100);
      setWorkflows(res.data.items.map(w => ({
        id: w.id,
        name: w.name,
        appType: w.app_type,
  ...
  const fetchTasks = useCallback(async () => {
    try {
      const res = await adminApi.getAdminDifyTasks(1, 100);
  ...
  const fetchLogs = useCallback(async () => {
    try {
      const res = await adminApi.getAdminRunLogs(1, 100);
  ...
  const handleDeleteTask = async (taskId: number, status: string) => {
    ...
    try {
      await adminApi.deleteAdminDifyTask(taskId);
  ...
  const handleTaskClick = async (task: Task) => {
    if (!task.viewed) {
      try {
        await adminApi.markAdminTaskViewed(task.id);
  ...
  const handleRun = async (wf: Workflow, inputs: Record<string, string>) => {
    try {
      await adminApi.createAdminDifyTask(wf.id, inputs);
      await fetchTasks();
  ...
  const handleDelete = async (id: number, name: string) => {
    ...
    try {
      await adminApi.deleteAdminWorkflow(id);
  ...
  return (
    <div ref={pageRef} className="flex h-full bg-[#fafafa]">
      ...
      <div className={cn('flex-1 flex flex-col min-w-0 transition-all', showTaskPanel && 'mr-96')}>
        ...
        <div className="px-8 py-2 border-b bg-neutral-50/70 text-xs text-muted-foreground">
          审计日志用于查看工作流输入输出与调用结果；跨系统任务排队与状态处理请前往“任务中心”。
        </div>
                  ...
                  {workflows.map(wf => {
                    ...
                    return (
                      <div key={wf.id}
                        ...
                        <div className="absolute top-2 right-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                          ...
                          {openDropdown === wf.id && (
                            <div className="absolute right-0 top-full mt-1 w-28 rounded-lg border bg-white shadow-lg py-1 z-20">
                              ...
                              <button
                                onClick={() => { handleDelete(wf.id, wf.name); setOpenDropdown(null); }}
                        ...
                        {wf.description && (
                          <p className="text-[11px] text-muted-foreground mb-1 line-clamp-2 w-full">{wf.description}</p>
                        )}
                        {wf.createdByName && (
                          <p className="text-[10px] text-gray-400 mb-1">
                            <a href={`/admin/users/${wf.createdBy}`} className="hover:underline hover:text-blue-500" onClick={e => e.stopPropagation()}>
                              {wf.createdByName}
                            </a>
                          </p>
                        )}
              ...
              <div className="bg-white rounded-xl border border-neutral-200/60 overflow-hidden">
                <table className="w-full text-sm">
                  <thead className="border-b border-neutral-100">
                    <tr>
                      {['工作流', '用户', '状态', '输入参数', '输出结果', '运行时间', '耗时'].map(h => (
                      ...
                      return (
                        <tr key={log.id} className="hover:bg-neutral-50/50 transition-colors">
                          ...
                          <td className="px-5 py-3.5 text-xs">
                            {log.user_name ? (
                              <a href={`/admin/users/${log.user_id}`} className="text-blue-600 hover:underline" onClick={e => e.stopPropagation()}>
                                {log.user_name}
                              </a>
                            ) : (
      ...
      {/* ---- Task Drawer ---- */}
      {showTaskPanel && (
        <div className="fixed right-0 top-0 bottom-0 w-96 bg-white border-x border-neutral-200/60 shadow-xl z-40 flex flex-col">
                    ...
                    <button onClick={() => handleTaskClick(task)}
                      ...
                      {task.user_name && (
                        <div className="text-[10px] text-gray-400 mb-1">
                          <a href={`/admin/users/${task.user_id}`} className="hover:underline hover:text-blue-500" onClick={e => e.stopPropagation()}>
                            {task.user_name}
                          </a>
                        </div>
                      )}
...
function AddWorkflowModal({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  const handleImportDSL = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        const nodes = (dsl as any)?.app?.workflow?.graph?.nodes || (dsl as any)?.workflow?.graph?.nodes || (dsl as any)?.workflow?.nodes || (dsl as any)?.graph?.nodes;
        ...
        if (!name.trim()) setName((dsl as any)?.app?.name || (dsl as any)?.workflow?.name || file.name.replace(/\.(yml|yaml|json)$/, ''));
  ...
  const handleAdd = async () => {
    if (!name.trim() || !apiKey.trim()) { toast.error('请填写名称和 API Key'); return; }
    const vars = fields.map(f => f.variable.trim()).filter(Boolean);
    if (new Set(vars).size !== vars.length) { toast.error('变量名不能重复'); return; }
    ...
    try {
      await adminApi.createAdminWorkflow({
...
function EditWorkflowModal({ workflow, onClose, onSaved }: { workflow: Workflow; onClose: () => void; onSaved: () => void }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (updateKey && apiKey.trim()) {
        payload.api_key = apiKey.trim();
      }
      await adminApi.updateAdminWorkflow(workflow.id, payload);
      toast.success('保存成功');
      onSaved();
    } catch (e: unknown) {
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl border w-[460px] overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-4">
          <div>
            <label className="text-xs font-medium text-muted-foreground mb-1.5 block">API Key（留空则不修改）</label>
            {!updateKey ? (
              <button onClick={() => setUpdateKey(true)} className="text-sm text-blue-600 hover:underline">
                点击修改 API Key
              </button>
            ) : (
              <input type="password" value={apiKey} onChange={e => setApiKey(e.target.value)} placeholder="app-xxx-xxx"
...
function RunWorkflowModal({ workflow, onClose, onRun }: { workflow: Workflow; onClose: () => void; onRun: (inputs: Record<string, string>) => Promise<void> }) {
  ...
  useEffect(() => {
    ...
    for (const [, val] of Object.entries(schema)) {
      const v = val as any;
      defaults[v.default ? Object.keys(schema).find(k => (schema as any)[k] === val) || '' : ''] = v.default || '';
    }
    // Simpler approach
    const d: Record<string, string> = {};
    for (const [key, val] of Object.entries(schema)) {
      d[key] = (val as any).default || '';
    }
    setFormData(d);
  }, [schema]);
  ...
  const handleRun = async () => {
    ...
    setRunning(true);
    try { await onRun(formData); } catch (e: unknown) {
