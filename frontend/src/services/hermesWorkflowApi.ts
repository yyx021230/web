...
export interface HermesReferenceExample { id: number; title: string; content: string; image_url?: string; preview_note?: string; source_section_title?: string; classification_reason?: string; style_tags?: string[]; requires_quote_data?: boolean; source_task_id?: number; visual_reviewed?: boolean; reference_family?: string; kind: 'copy' | 'image'; }
export interface HermesReferenceType { id: string; name: string; description: string; structure: string; accent: string; reference_count: number; preview_count: number; requires_quote_data?: boolean; quote_reference_count?: number; production_reference_count?: number | null; production_without_quote_count?: number | null; examples: HermesReferenceExample[]; }
