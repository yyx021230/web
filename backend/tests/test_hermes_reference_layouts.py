...
async def test_catalog_uses_real_layouts_and_shared_source_sections(client, monkeypatch, tmp_path):
    ...
    assert types['cards']['examples'][0]['id'] == 335
    assert types['table']['examples'][0]['id'] == 554
    assert types['collage']['examples'][0]['id'] == 568
    assert types['diagram']['examples'][0]['id'] == 332
    assert {r['id'] for r in types['hero']['examples']} == {353, 535}
    assert types['headline']['examples'][0]['id'] == 573
    note = next(r for r in types['cards']['examples'] if r['id'] == 324)
    assert note['id'] == 324 and note['source_section_title'].startswith('方案 10')
    assert note['image_url'] == '' and note['preview_note']
    assert types['cards']['requires_quote_data'] is True
