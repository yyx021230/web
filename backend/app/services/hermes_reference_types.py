...
TAXONOMY_VERSION = 'reference-types-v4-curated'
COPY_TYPES = [
...
IMAGE_TYPES = [
    {'id': 'hero', 'name': '单图主视觉', 'description': '一张完整车图为核心，场景与棚拍都可以', 'structure': '保留镜头、车位、标题层级和留白', 'accent': 'blue'},
    {'id': 'headline', 'name': '大字主视觉', 'description': '大字、金额或日期成为主要视觉元素', 'structure': '保留巨字占比、字效和与车辆的叠压关系', 'accent': 'lavender'},
    {'id': 'collage', 'name': '多图拼贴', 'description': '两张以上独立车图、照片或分镜组合', 'structure': '保留照片数量、分区和主次关系，不把文字卡当作多图', 'accent': 'sage'},
    {'id': 'table', 'name': '表格对照', 'description': '共同表头和行列组织配置、费用或事项', 'structure': '保留表头与行列；含多个配置报价的母版另需完整价格', 'accent': 'sand'},
    {'id': 'cards', 'name': '分块卡片', 'description': '卡片、便签或分区分别承载信息', 'structure': '保留卡片位置、数量与层级；报价与非报价分别判断资料', 'accent': 'peach'},
    {'id': 'list', 'name': '清单笔记', 'description': '纵向文字条目、备忘录或说明单', 'structure': '保留章节、条目、手写或文档式排版', 'accent': 'sage'},
    {'id': 'dialog', 'name': '对话问答', 'description': '头像与聊天气泡组织一问一答', 'structure': '保留对话顺序、气泡位置和附图，不冒充真实客户证明', 'accent': 'blue'},
    {'id': 'diagram', 'name': '图解科普', 'description': '引线、结构示意或流程分支解释知识', 'structure': '保留标注关系和信息分支，只使用有依据的内容', 'accent': 'sand'},
]
PRIMARY_FROM_LEGACY = {
    'scene_poster': 'hero', 'studio_poster': 'hero', 'headline_poster': 'headline',
    'price_highlight': 'headline', 'quote_table': 'table', 'quote_cards': 'cards',
    'photo_collage': 'collage', 'note_poster': 'cards', 'feature_infographic': 'diagram',
}


def prompt_fingerprint(row: dict[str, Any]) -> str:
    text = str(row.get('chinese') or row.get('chinese_example') or '').strip()
    return hashlib.sha256(text.encode()).hexdigest()


@lru_cache(maxsize=1)
def curated_registry() -> dict[str, dict[str, Any]]:
    """Versioned, reviewed facts about layouts; never trust a client type override.

    A prompt edit invalidates its fingerprint. Source text is not rewritten to
    smuggle keywords into the classifier. Both API and Worker ship this file.
    """
    path = Path(__file__).resolve().parents[1] / 'data/hermes_image_curation.json'
    if not path.is_file():
        return {}
    data = json.loads(path.read_text(encoding='utf-8'))
    if data.get('version') != 1:
        raise ValueError('Unsupported image curation registry')
    return data['by_prompt_sha256']


def curated_source(row: dict[str, Any]) -> dict[str, Any]:
    return curated_registry().get(prompt_fingerprint(row), {})


def dedupe_image_sources(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """One entry per confirmed family or exact prompt. Originals stay in the DB.

    Do not call visually similar but unreviewed sources duplicates. Preferred
    representatives win only among the supplied (possibly eligible) sources.
    """
    chosen: dict[str, dict[str, Any]] = {}
    for row in rows:
        meta = curated_source(row)
        if meta.get('action') in {'exclude', 'reserve', 'not_selected'}:
            continue
        key = meta.get('family') or (prompt_fingerprint(row) if row.get('chinese') or row.get('chinese_example') else f'unstructured:{id(row)}')
        previous = chosen.get(key)
        priority = (meta.get('action') == 'keep', meta.get('preferred_image_path') == urlsplit(str(row.get('image_url') or '')).path, meta.get('action') != 'merge', meta.get('preferred_library_id') == row.get('id'), bool(meta.get('visual_reviewed')))
        old = curated_source(previous) if previous else {}
        old_priority = (old.get('action') == 'keep', old.get('preferred_image_path') == urlsplit(str((previous or {}).get('image_url') or '')).path, old.get('action') != 'merge', old.get('preferred_library_id') == (previous or {}).get('id'), bool(old.get('visual_reviewed')))
        if previous is None or priority > old_priority:
            chosen[key] = row
    result = list(chosen.values())
    return rows if len(result) == len(rows) and all(a is b for a, b in zip(rows, result)) else result
COPY_CUES = {
...
def legacy_image_layout_analysis(row: dict[str, Any]) -> dict[str, Any]:
...
def image_layout_analysis(row: dict[str, Any]) -> dict[str, Any]:
    legacy = legacy_image_layout_analysis(row)
    meta = curated_source(row)
    if meta.get('group'):
        return {**legacy, 'type': meta['group'], 'reason': meta['reason'],
                'requires_quote_data': bool(legacy['requires_quote_data'] or meta.get('requires_quote_data')),
                'visual_reviewed': bool(meta.get('visual_reviewed')),
                'source_task_id': meta.get('source_task_id'), 'family': meta.get('family'),
                'curation_action': meta.get('action'), 'legacy_type': legacy['type'],
                'production_block_reason': meta.get('production_block_reason')}
    raw = str(row.get('chinese') or row.get('chinese_example') or '')
    text = positive_layout_text(raw)
    has = lambda pattern: re.search(pattern, text, re.I) is not None
    if '包含多个完整方案' in legacy['reason']:
        category, reason = None, legacy['reason']
    elif has(r'聊天.{0,8}(?:截图|界面|气泡)|(?:左右|一问一答).{0,12}(?:气泡|对话)|(?:绿色|白色|灰色).{0,8}(?:聊天|对话|消息)气泡'):
        category, reason = 'dialog', '头像、消息气泡和问答顺序组织画面'
    elif legacy['type'] == 'feature_infographic' or has(r'思维导图|流程图|树状|分支.{0,14}(?:箭头|连接|线条)|(?:动力|原理|车身).{0,12}(?:剖面|透视图)|(?:引线|尺寸线|标注线|指引线|箭头标注)|功能位置|按键位置'):
        category, reason = 'diagram', '引线、结构示意或分支关系组织信息'
    elif has(r'表头|单元格|(?:三|四|五|六|3|4|5|6)列.{0,10}(?:表格|数据)|(?:完整|规整|对比|明细|报价|价格).{0,8}表格|表格.{0,12}(?:行列|横线|纵线)'):
        category, reason = 'table', '共同表头及行列承载对照信息，报价是资料条件而非视觉分类'
    elif has(r'备忘录|会议纪要|(?:纸质|文件式).{0,8}(?:通知|说明|宣传单)|纯文字.{0,10}(?:清单|信息海报)|(?:编号|纵向|分章|分段).{0,12}(?:清单|笔记|条目)'):
        category, reason = 'list', '连续章节和纵向文字条目是主要结构'
    elif legacy['type'] in {'photo_collage', 'note_poster', 'quote_cards', 'quote_table', 'feature_infographic'}:
        category, reason = PRIMARY_FROM_LEGACY[legacy['type']], legacy['reason']
    elif has(r'(?:两|二|三|四|五|六|2|3|4|5|6)(?:张|幅|个).{0,8}(?:照片|细节图|独立画面)|上下.{0,10}(?:双角度|双画面)|漫画.{0,8}分镜'):
        category, reason = 'collage', '多个独立照片或分镜组成画面'
    elif has(r'(?:四|三|两|二|双|4|3|2)(?:张|块|个|组|项|栏|联).{0,15}(?:信息卡|权益卡|便利贴|便签|配置模块)|(?:卡片|信息框).{0,16}(?:并排|排布|排列)|(?:左右|两|双).{0,8}(?:信息栏|明细框)|卡片之间|卡片内部'):
        category, reason = 'cards', '独立卡片或文字分区构成主要信息载体'
    else:
        category, reason = PRIMARY_FROM_LEGACY.get(legacy['type']), legacy['reason']
    return {**legacy, 'type': category, 'reason': reason, 'legacy_type': legacy['type'], 'visual_reviewed': False,
            'production_block_reason': reason if '包含多个完整方案' in legacy['reason'] else None}
...
def matching_pool(rows: list[dict[str, Any]], kind: str, requested: str | None, classifier=None) -> list[dict[str, Any]]:
    if not requested or requested in {'跟随母文结构', '跟随母图结构', 'auto'}:
        return dedupe_image_sources(rows) if kind == 'image' else rows
    definitions = COPY_TYPES if kind == 'copy' else IMAGE_TYPES
    aliases = LEGACY_COPY if kind == 'copy' else LEGACY_IMAGE
    legacy_allowed = (aliases.get(requested) or {d['id'] for d in LEGACY_IMAGE_TYPES if requested in {d['id'], d['name']}}) if kind == 'image' else set()
    allowed = {d['id'] for d in definitions if requested in {d['id'], d['name']}} or aliases.get(requested) or legacy_allowed
    if not allowed:
        raise ValueError(f'不支持的{kind}类型：{requested}')
    classify = classify_copy if kind == 'copy' else classify_image
    primary_request = kind != 'image' or any(requested in {d['id'], d['name']} for d in IMAGE_TYPES)
    matched = [row for row in rows if (classify(row) if primary_request else legacy_image_layout_analysis(row)['type']) in allowed]
    if kind == 'image' and classifier and not primary_request:
        # Preserve the old fine-grained request contract for saved/retried runs.
        matched = [row for row in matched if
                   (classifier(row) == 'multi_config_quote') == legacy_image_layout_analysis(row)['requires_quote_data']
                   or (legacy_image_layout_analysis(row)['type'] == 'note_poster' and row.get('source_section_title'))]
    return dedupe_image_sources(matched) if kind == 'image' else matched
...
def type_requires_quote_data(requested: str | None) -> bool:
    return requested == '多配置报价单' or any(d.get('requires_quote_data') and requested in {d['id'], d['name']} for d in LEGACY_IMAGE_TYPES)
