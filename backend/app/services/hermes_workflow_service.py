...
class HermesWorkflowService:
    ...
    async def create_run(
        ...
        *,
        ...
        primary_type = next((d for d in IMAGE_TYPES if image_type in {d['id'], d['name']}), None)
        if primary_type and requested_by:
            from app.models.user import User
            from app.services.hermes_reference_service import reference_catalog
            operator = await self.db.get(User, requested_by)
            if operator:
                catalog = await reference_catalog(self.db, operator)
                entry = next(t for t in catalog['image_types'] if t['id'] == primary_type['id'])
                for model in model_names:
                    complete_prices = any(p['vehicle_model'] == model and p.get('allow_multi_config_quote') and p.get('quote_rows') for p in snapshots)
                    field = 'production_reference_count' if complete_prices else 'production_without_quote_count'
                    if entry.get(field) == 0:
                        raise HTTPException(status_code=400, detail=f'{model}当前资料条件下，{primary_type["name"]}暂无可生产母版；可浏览原图或更换版式')
        run = HermesWorkflowRun(
            ...
            parameters={
                ...
                "selection_contract": {
                    ...
                    "image_label": next((d['name'] for d in IMAGE_TYPES + LEGACY_IMAGE_TYPES if d['id'] == image_type), image_type),
