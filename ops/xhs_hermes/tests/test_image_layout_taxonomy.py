...
def test_real_library_layout_regressions(row):
    ...
    expected = {503: 'dialog', 325: 'table'}.get(row['id'], catalog.PRIMARY_FROM_LEGACY.get(row['expected']))
    assert catalog.classify_image(row) == expected
    assert row == before
...
def test_discarded_notebook_style_is_not_positive_evidence(negative):
    ...
    assert catalog.classify_image(row) == 'cards'
...
def test_non_photo_splices_do_not_become_collages(text):
    assert catalog.classify_image({'chinese': text}) == 'hero'


def test_preview_only_override_cannot_change_worker_classification():
    row = next(r for r in FIXTURES if r['id'] == 335)
    assert catalog.classify_image({**row, 'preview_type': 'studio_poster'}) == 'cards'
...
def test_approved_section_is_identical_for_preview_and_generation():
    ...
    assert catalog.classify_image(preview) == catalog.classify_image(production) == 'cards'
