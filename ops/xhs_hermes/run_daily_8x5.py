...
NEGATIVE_PROMPT = (
    "文字模糊，错别字，乱码，联系方式，二维码，站外导流，旗帜，夸张促销语，"
...
def image_plan_instruction(
    ...
    if quote_mode:
        quote_rule = (
            ...
            "补贴后金额必须带‘按相应条件测算’语义，不能写成"
            "无条件成交价或落地价。报价母版允许2至30个真实可见文字槽；文字槽不是版式载体，"
...
硬要求：
1. 母图是完整复刻蓝图，不只是风格参考。复刻其整体构图、镜头、车辆占比、文字槽数量与顺序、每槽句式和功能、字号层级、对齐、配色、装饰以及CTA位置；场景只轻微改变一个维度。
2. 主体车型改为“{case['vehicle_model']}”，品牌为“{case['brand']}”。车辆外形、车标、灯组、轮毂和比例严格跟随随后输入的车型库辅助图；母图车辆只提供镜头和摆位。
3. 对母图文字逐槽做最小替换：不涉及旧汽车事实的通用标题、小标签、情绪句、说明句和安全CTA，能原样保留就原样保留。只替换旧品牌/车型、过期时间活动、无依据金额配置参数政策、明确禁用词；禁止为了“更简洁”整套重写母图文字。
...
7. {quote_rule}
8. 母图若有安全的咨询/留城市CTA，保留其句式、位置和小字层级，只替换不安全词或错误汽车信息；不要把CTA擅自换成重复品牌名。
