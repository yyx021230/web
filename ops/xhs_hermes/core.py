...
def safe_prompt_pool(items: Iterable[dict[str, Any]], *, allow_quote_table: bool) -> list[dict[str, Any]]:
    ...
    for raw in items:
        ...
        # The visual family and the financial data requirement are independent.
        # Curated facts may add quote requirements, never remove the old guard.
        analysis = catalog.image_layout_analysis(raw)
        if analysis.get('production_block_reason'):
            continue
        if analysis['requires_quote_data']:
            template_type = QUOTE_TABLE_TYPE
        if not allow_quote_table and template_type == QUOTE_TABLE_TYPE:
            continue
        if not allow_quote_table and UNSAFE_QUOTE_TEMPLATE_RE.search(prompt):
            continue
        reviewed_nonposter = analysis.get('visual_reviewed') and analysis['type'] in {'dialog', 'list', 'diagram'} and re.search(r'聊天|对话|截图|备忘录|清单|信息图', prompt)
        if (not re.search(r"海报|封面", prompt) and not reviewed_nonposter) or not re.search(r"文字|标题|文案|字体|LOGO|logo", prompt):
            continue
        ...
        row["image_primary_type"] = analysis['type']
        result.append(row)
    return catalog.dedupe_image_sources(result)
...
def validate_copy(
    *,
    ...
    if case.get("vehicle_stage") == "new" and not case.get("publication_constraints") and re.search(r"有现车|现车可提", full):
...
def validate_image_plan(plan: dict[str, Any], copy: dict[str, Any], case: dict[str, Any]) -> list[str]:
    ...
    lead_text = "\n".join([prompt, *blocks])
    if IMAGE_LEAD_RE.search(lead_text):
        errors.append("图片不得出现留咨、互动或获取资料话术")
    errors.extend(policy_constraint_errors("\n".join(blocks), case))
...
def image_ocr_errors(
    ...
    *,
        ...
        if isinstance(raw, dict):
            ...
            if confidence < min_confidence:
                is_hard_fact = (
                    ...
                    or IMAGE_LEAD_RE.search(text) is not None
    ...
    target_code = target.replace("零跑", "").replace(" ", "")
    errors: list[str] = []
    errors.extend('OCR政策回查：' + error for error in policy_constraint_errors(full, case))
    banned = sorted({term for term in EXACT_BANNED_TERMS if term in full})
    if banned:
        errors.append("OCR识别到禁用词：" + "、".join(banned))
    if IMAGE_LEAD_RE.search(full):
        errors.append("OCR识别到图片留咨、互动或获取资料话术")
    competitors = sorted(set(COMPETITOR_RE.findall(full)))
    if competitors:
